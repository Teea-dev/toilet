...
export default function Map({
  ...
  function getColorByStatus(toilet: ToiletDataFormat) {
  ...
  useEffect(() => {
    ...
    data.forEach((toilet) => {
      const el = document.createElement("div");
      el.className = getColorByStatus(toilet);

      el.addEventListener("click", () => {
        handleMarkerClick(toilet.name);
      });

      const cleanlinessDisplay =
        toilet.cleaniness_rating !== undefined
          ? toilet.cleaniness_rating.toFixed(1)
          : "No rating";
      ...
      const popup = new mapboxgl.Popup({ offset: 25 }).setHTML(`
        ...
        ${
          toilet.opening_time && toilet.closing_time
            ? `<p><strong>Hours:</strong> ${toilet.opening_time} - ${toilet.closing_time}</p>`
            : ""
        }
      `);
